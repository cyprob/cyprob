...
//
//	💡 Suggestions:
//	  → Retry with different source:  cyprob plugin update --source github
//	  → Force re-download:            cyprob plugin update --force
...
//
//	💡 Suggestions:
//	  → List available plugins:  cyprob plugin list
//	  → Try GitHub source:       cyprob plugin install ssh-weak-cipher --source github
...
var suggestionGenerators = map[string]func(string) []string{
	"PLUGIN_NOT_FOUND": func(operation string) []string {
		return []string{
			"List available plugins:  cyprob plugin list",
			fmt.Sprintf("Try GitHub source:       cyprob plugin %s <plugin> --source github", operation),
		}
	},
	"SERVICE_UNAVAILABLE": func(operation string) []string {
		return []string{
			fmt.Sprintf("Retry with GitHub:       cyprob plugin %s --source github", operation),
			"Check network connection",
		}
	},
	"CHECKSUM_MISMATCH": func(operation string) []string {
		return []string{
			fmt.Sprintf("Force re-download:       cyprob plugin %s --force", operation),
		}
	},
	"VERSION_CONFLICT": func(operation string) []string {
		return []string{
			"Uninstall first:         cyprob plugin uninstall <plugin>",
			fmt.Sprintf("Force reinstall:         cyprob plugin %s --force", operation),
		}
	},
	"PARTIAL_FAILURE": func(operation string) []string {
		return []string{
			fmt.Sprintf("See full details:        cyprob plugin %s --output json", operation),
		}
	},
	"NO_PLUGINS_FOUND": func(operation string) []string {
		return []string{
			...
			fmt.Sprintf("Try GitHub source:       cyprob plugin %s <name> --source github", operation),
			fmt.Sprintf("Force re-download:       cyprob plugin %s --force", operation),
			"List cached plugins:     cyprob plugin list",
		}
	},
	"INVALID_CATEGORY": func(string) []string {
		return []string{
			"Valid categories: ssh, http, tls, database, network, web, iot, misc",
			"List plugins:     cyprob plugin list",
		}
	},
	...
	"PLUGIN_NOT_INSTALLED": func(string) []string {
		return []string{
			"List installed:   cyprob plugin list",
			"Install plugin:   cyprob plugin install <plugin>",
		}
	},
	"INVALID_TARGET": func(string) []string {
		return []string{
			"Provide a target:           cyprob scan 192.168.1.0/24",
			"Scan multiple hosts:        cyprob scan 10.0.0.1 10.0.0.2",
		}
	},
	"CONFLICTING_DISCOVERY_FLAGS": func(string) []string {
		return []string{
			"Remove either --only-discover or --no-discover",
			"Run help for options:       cyprob scan --help",
		}
	},
	"SCAN_FAILURE": func(string) []string {
		return []string{
			"Retry with verbose logs:    cyprob scan <target> --verbose",
			"Enable progress output:     cyprob scan <target> --progress",
		}
	},
	...
	"SERVER_INVALID_PORT": func(string) []string {
		return []string{
			...
			"Example:                 cyprob server start --port 8080",
		}
	},
	"SERVER_INVALID_CONCURRENCY": func(string) []string {
		return []string{
			"Set jobs concurrency to at least 1",
			"Example:                 cyprob server start --jobs-concurrency 4",
		}
	},
	...
	"SERVER_CONFIG_UNAVAILABLE": func(string) []string {
		return []string{
			"Run via the cyprob CLI so AppManager initializes",
	...
	"SERVER_STORAGE_INIT_FAILED": func(string) []string {
		return []string{
			...
			"Override storage root:     cyprob server start --storage-dir <path>",
		}
	},
	"SERVER_PLUGIN_INIT_FAILED": func(string) []string {
		return []string{
			"Check plugin cache directory access",
			"Retry after running:      cyprob plugin clean",
		}
	},
	"SERVER_INIT_FAILED": func(string) []string {
		return []string{
			"Retry with verbose logging: cyprob server start --verbose",
	...
	"FINGERPRINT_SOURCE_REQUIRED": func(string) []string {
		return []string{
			...
			"Example:                   cyprob fingerprint sync --url https://example/catalog.yaml",
		}
	},
	...
	"FINGERPRINT_STORAGE_DISABLED": func(string) []string {
		return []string{
			"Set cache directory:       cyprob fingerprint sync --cache-dir <path>",
	...
	"DAG_INVALID": func(string) []string {
		return []string{
			"Run cyprob dag validate on the export output",
