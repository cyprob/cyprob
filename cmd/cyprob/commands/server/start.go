// Package server provides the Cobra command implementation for the Cyprob server lifecycle.
...
// newStartServerCommand creates and returns the 'cyprob server start' command.
//
// This command initializes the Cyprob server runtime, which includes:
...
//	cyprob server start --workspace-dir /data/cyprob --jobs-concurrency 10
//
// See NOTES.md#30 for detailed server architecture design.
func newStartServerCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "start",
		Short: "Start the Cyprob server",
		Long: `Start the Cyprob server process.
