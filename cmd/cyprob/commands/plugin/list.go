...
found in the plugin cache directory.`,
  ...
  cyprob plugin list --verbose

  # List plugins from custom cache directory
  cyprob plugin list --cache-dir /custom/path

  # JSON output
  cyprob plugin list --output json`,
...
func printEmptyPluginList(f format.Formatter) error {
	...
	return f.PrintSummary("  cyprob plugin install <plugin-name>")
}
