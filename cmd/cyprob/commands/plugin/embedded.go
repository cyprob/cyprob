...
func newEmbeddedCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `List all security check plugins embedded in the Cyprob binary.
...
for common security issues across SSH, HTTP, TLS, Database, and Network protocols.`,
  ...
  cyprob plugin embedded

  # List embedded plugins with detailed information
  cyprob plugin embedded --verbose

  # List only SSH plugins
  cyprob plugin embedded --category ssh

  # List TLS plugins with details
  cyprob plugin embedded --category tls --verbose

  # JSON output
  cyprob plugin embedded --output json`,
